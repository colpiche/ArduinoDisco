...
void loop()
{
  ...
  float f = Approx_FFT(isin_data, 256, 100);
  Serial.println(f);
  // delay(99999);
  delay(100);
}
...
1. samples[]        : Data array,
2. samples_quantity : Number of sample (recommended sample size 2,4,8,16,32,64,128,256,512...)
...
float Approx_FFT(int samples[], int samples_quantity, float frequency)
{
  Serial.println("Approx_FFT called");
  int coeff_quantity, c1, f, level, x, data_max, data_min = 0;
  ...
  {
    if (Pow2[i] <= samples_quantity)
    {
      level = i;
    }
  }
  coeff_quantity = Pow2[level];

  Serial.print("Working with ");
  Serial.print(coeff_quantity);
  Serial.println(" coefficients.");
  
  int out_r[coeff_quantity];  // real part of transform
  int out_im[coeff_quantity]; // imaginory part of transform

  for (int i = 0; i < coeff_quantity; i++) // getting min max and average for scalling
  {
    out_r[i] = 0;
    out_im[i] = 0;
    data_avg = data_avg + samples[i];
    if (samples[i] > data_max)
    {
      data_max = samples[i];
    }
    if (samples[i] < data_min)
    {
      data_min = samples[i];
    }
  }

  data_avg = data_avg >> level;
  scale = 0;
  ...
  if (data_mag > 1024)
  {
    for (int i = 0; i < coeff_quantity; i++)
    {
      samples[i] = samples[i] - data_avg;
  ...
  x = 0;
  for (int b = 0; b < level; b++) // bit reversal order stored in im_out array
  {
    c1 = Pow2[b];
    f = Pow2[level] / (c1 + c1);
    for (int j = 0; j < c1; j++)
  ...
  for (int i = 0; i < coeff_quantity; i++) // update input array as per bit reverse order
  {
    out_r[i] = samples[out_im[i]];
    out_im[i] = 0;
  }
  ...
  float e;
  int c, s, temp4;
  for (int i = 0; i < level; i++) // fft
  {
    i10 = Pow2[i];               // overall values of sine/cosine
    i11 = Pow2[level] / Pow2[i + 1]; // loop with similar sine cosine
  ...
  */
  ...
  int fout, fm, fstp;
  float fstep;
  fstep = frequency / samples_quantity;
  fstp = fstep;
  fout = 0;
  fm = 0;

  for (int i = 1; i < Pow2[level - 1]; i++) // getting amplitude from compex number
  {
  ...
  return (fstep * frequency / samples_quantity);
}
